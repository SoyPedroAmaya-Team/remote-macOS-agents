...
main() {
    ...
    case "$command" in
        list)
            ...
            ;;
        start)
            [[ -n "$1" ]] || { log_error "Usage: $0 start <name>"; exit 1; }
            start_job "$1"
            ;;
        stop)
            [[ -n "$1" ]] || { log_error "Usage: $0 stop <name>"; exit 1; }
            stop_job "$1"
            ;;
        restart)
            [[ -n "$1" ]] || { log_error "Usage: $0 restart <name>"; exit 1; }
            restart_job "$1"
            ;;
        logs)
            [[ -n "$1" ]] || { log_error "Usage: $0 logs <name> [lines]"; exit 1; }
            view_logs "$1" "${2:-50}"
            ;;
        status)
            [[ -n "$1" ]] || { log_error "Usage: $0 status <name>"; exit 1; }
            job_status "$1"
            ;;
        add)
            add_job "$1"
            ;;
        remove|delete)
            [[ -n "$1" ]] || { log_error "Usage: $0 remove <name>"; exit 1; }
            remove_job "$1"
            ;;
        -h|--help|help)
            show_help
